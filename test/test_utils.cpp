...
std::map<uint8_t, std::string> I3CBusMap::i3cBusMap = {
    {1, "bus1"},
    {2, "bus2"}
};
...
TEST_F(AspeedTests, FindRootBusNo_Success) {
    EXPECT_EQ(findRootBusNo("1e7a4000.i3c2/i3c-1/1-4cd092c310a/i3c-2/2-4cd092c310b"), 2);
}

TEST_F(AspeedTests, FindRootBusNo_InvalidPath) {
    EXPECT_THROW(findRootBusNo("1e7a4000.i3c2"), std::runtime_error);
}

TEST_F(AspeedTests, ReadRootBusName_Success) {
    std::ofstream("name") << "3-4cd15771616.tp3";
    std::string newPath = currentPath + "/";
    EXPECT_EQ(readRootBusName(newPath), "tp3");
    std::remove("name");
}

TEST_F(AspeedTests, ReadRootBusName_ThrowsWhenNoSlash) {
    EXPECT_THROW(readRootBusName("invalidpath"), std::runtime_error);
}

TEST_F(AspeedTests, ReadRootBusName_ThrowsWhenNoDot) {
    std::ofstream("name") << "3-4cd15771616";
    EXPECT_THROW(readRootBusName(currentPath), std::runtime_error);
}
...
{
    std::string dirPath = "1e7a4000.i3c2/i3c-1/1-4cd092c310a/i3c-2/2-4cd092c310b";
    ...
    file.close();
    file1.close();
    std::string hubPath = currentPath + "/1e7a4000.i3c2/i3c-1/1-4cd092c310a/i3c-2/2-4cd092c310b";
...
{
    std::string hubPath1 = currentPath + "/1e7a4000.i3c2/i3c-1/1-4cd092c310a/i3c-2/2-4cd092c310b";
    std::string hubPath2 = currentPath + "/1e7a4000.i3c2/i3c-1/1-4cd092c310a/i3c-2/2-4cd092c310c";
    hubList[hubPath1] = std::make_pair(HubInfo{1, 1, "RootBus1", "TPConf1", 0},
